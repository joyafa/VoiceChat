...
HANDLE g_hHardWareEventObject = INVALID_HANDLE_VALUE;
...
void  RecvThreadFunction(LPVOID lpParameter   )
{
	...
	g_hHardWareEventObject=CreateEvent(NULL,TRUE,TRUE,NULL);
	HIDOverlapped.hEvent=g_hHardWareEventObject;
	...
	{
		ResetEvent(g_hHardWareEventObject);
		ZeroMemory(recvDataBuf,64);
		ReadFile(
		...
		DWORD dwRet = WaitForSingleObject(g_hHardWareEventObject,5000);
...
void  SendThreadFunction(LPVOID lpParameter)
{
	...
	while(!pUsbDevice->m_bExitSendThread)
	{
		...
		Result=GetLastError();
		Result=WaitForSingleObject(g_hHardWareEventObject,1000);
